...
class MNISTMetricDataset(Dataset):
    ...
    def _sample_negative(self, index):
        ...
        negative_ids = list(self.target2indices.keys())
        negative_ids.remove(target_id)

        indices = self.target2indices[choice(negative_ids)]
        return choice(indices)

    def _sample_positive(self, index):
        target_id = self.targets[index].item()
        indices = self.target2indices[target_id]
        return choice(indices)
